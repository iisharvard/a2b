...
const IcebergVisualization: React.FC<IcebergVisualizationProps> = ({ 
  ...
  // Update when party names change
  useEffect(() => {
    // Update party names when they change from props
    if (party1Name !== icebergData.party1.name || party2Name !== icebergData.party2.name) {
      console.log(`IcebergVisualization: Party names changed to ${party1Name} and ${party2Name}`);
      setIcebergData(prevData => ({
        ...prevData,
        party1: {
          ...prevData.party1,
          name: party1Name
  ...
  useEffect(() => {
    if (value) {
      const parsedData = parseMarkdown(value);
      
      // Ensure we're using the current party names, not necessarily what was parsed
      parsedData.party1.name = party1Name;
      parsedData.party2.name = party2Name;
      
      setIcebergData(parsedData);
    } else {
      const defaultData = {
        party1: {
          name: party1Name,
